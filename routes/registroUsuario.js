...
router.post("/login", (req, res) => {
  ...
  mysqlConnection.query(
    `SELECT * FROM Registro WHERE CorreoElectronico='${CorreoElectronico}' AND Contrasena='${Contrasena}'`,
    (err, rows, fields) => {
      if (err) {
      ...
      if (rows.length > 0) {
        ...
        res.json({
          message: `Correo electronico o contraseña errada, por favor verifique la información ingresada`
        });
      }
    }
  );
...
router.put("/cambiarContrasena/:ID_Usuario", (req, res) => {
  ...
  const { ID_Usuario } = req.params;
  mysqlConnection.query(
    `UPDATE Registro SET Contrasena=? where ID_Usuario=?`,
